...
def _pushhook(*args, **kwargs):
    ...
    user = User.get_by_username(kwargs['username'])
    fullname = '%s %s' % (user.name, user.lastname)
    email = user.email

    if kwargs['scm'] == 'git':
        for f in repo.extra_fields:
            if f.field_key == 'webhook' and f.field_value:
                url = f.field_value

                print('Triggering OpenShift WebHook:', url)

                payload = {}
                payload['type'] = 'git'

                payload['git'] = dict(
                    uri=kwargs['server_url'],
                    refs='',
                    commit=kwargs['pushed_revs'][-1],
                    author=dict(name=fullname, email=email),
                    committer=dict(name=fullname, email=email),
                    message=''
                )
